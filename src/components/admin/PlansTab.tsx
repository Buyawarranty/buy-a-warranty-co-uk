...
export const PlansTab = () => {
  ...
  const addCoverageItem = () => {
    ...
    setEditingPlan({
      ...editingPlan,
      coverage: [...editingPlan.coverage, newCoverageItem]
    });
    setNewCoverageItem('');
  };
  ...
  const addAddOnItem = () => {
    ...
    setEditingPlan({
      ...editingPlan,
      add_ons: [...editingPlan.add_ons, newAddOnItem]
    });
    setNewAddOnItem('');
  };
  ...
  const removeAddOnItem = (index: number) => {
    ...
    setEditingPlan({
      ...editingPlan,
      add_ons: editingPlan.add_ons.filter((_, i) => i !== index)
    });
  };
                      ...
                      {editingPlan && (
                        <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {editingPlan.add_ons.map((item, index) => (
                                <div key={index} className="flex items-center space-x-2">
                                  <span className="flex-1 text-sm bg-gray-50 p-2 rounded">{item}</span>
                                  <Button
                                    variant="ghost"
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    ...
                    {plan.coverage.length > 3 && (
                      <li className="text-xs text-gray-500">+{plan.coverage.length - 3} more</li>
                    )}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    {plan.add_ons.slice(0, 2).map((item, index) => (
                      <li key={index} className="flex items-center">
                        <span className="w-2 h-2 bg-orange-500 rounded-full mr-2"></span>
                        {item}
                      </li>
                    ))}
                    {plan.add_ons.length > 2 && (
                      <li className="text-xs text-gray-500">+{plan.add_ons.length - 2} more</li>
                    )}
                  </ul>
                </div>
              </div>
